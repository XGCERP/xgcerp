...
frappe.ui.form.on("Supplier", {
	setup: function (frm) {
		...
		frm.set_query("supplier_primary_contact", function (doc) {
			return {
				query: "xgcerp.buying.doctype.supplier.supplier.get_supplier_primary_contact",
				filters: {
					supplier: doc.name,
				},
			};
		...
		frm.make_methods = {
			"Bank Account": () => xgcerp.utils.make_bank_account(frm.doc.doctype, frm.doc.name),
			"Pricing Rule": () => xgcerp.utils.make_pricing_rule(frm.doc.doctype, frm.doc.name),
		};
	},
	...
	refresh: function (frm) {
		...
		if (frm.doc.__islocal) {
			...
			frm.add_custom_button(
				__("Bank Account"),
				function () {
					xgcerp.utils.make_bank_account(frm.doc.doctype, frm.doc.name);
				},
				__("Create")
			);
			...
			frm.add_custom_button(
				__("Pricing Rule"),
				function () {
					xgcerp.utils.make_pricing_rule(frm.doc.doctype, frm.doc.name);
				},
				__("Create")
			);
			...
			// indicators
			xgcerp.utils.set_party_dashboard_indicators(frm);
		}
	},
	...
	show_party_link_dialog: function (frm) {
		const dialog = new frappe.ui.Dialog({
			...
			primary_action: function ({ customer }) {
				frappe.call({
					method: "xgcerp.accounts.doctype.party_link.party_link.create_party_link",
					args: {
