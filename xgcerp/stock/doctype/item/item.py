...
import xgcerp
from xgcerp.controllers.item_variant import (
...
from xgcerp.stock.doctype.item_default.item_default import ItemDefault
from xgcerp.stock.utils import get_valuation_method
...
class Item(Document):
	...
	if TYPE_CHECKING:
		...
		from xgcerp.stock.doctype.item_barcode.item_barcode import ItemBarcode
		from xgcerp.stock.doctype.item_customer_detail.item_customer_detail import ItemCustomerDetail
		from xgcerp.stock.doctype.item_default.item_default import ItemDefault
		from xgcerp.stock.doctype.item_reorder.item_reorder import ItemReorder
		from xgcerp.stock.doctype.item_supplier.item_supplier import ItemSupplier
		from xgcerp.stock.doctype.item_tax.item_tax import ItemTax
		from xgcerp.stock.doctype.item_variant_attribute.item_variant_attribute import ItemVariantAttribute
		from xgcerp.stock.doctype.uom_conversion_detail.uom_conversion_detail import UOMConversionDetail
			...
			item_price = frappe.get_doc(
				{
					...
					"uom": self.stock_uom,
					"brand": self.brand,
					"currency": xgcerp.get_default_currency(),
			...
			for item_barcode in self.barcodes:
				...
				if item_barcode.barcode:
					...
					if item_barcode.barcode_type:
						barcode_type = convert_xgcerp_to_barcodenumber(
	...
	def recalculate_bin_qty(self, new_name):
		from xgcerp.stock.stock_balance import repost_stock
		...
		if self.has_variants:
			...
			if variants:
				if len(variants) <= 30:
					...
					frappe.enqueue(
						"xgcerp.stock.doctype.item.item.update_variants",
						variants=variants,
						template=self,
...
def convert_xgcerp_to_barcodenumber(xgcerp_number, barcode):
	if xgcerp_number == "EAN":
		ean_type = {
			8: "EAN8",
			13: "EAN13",
		}
	...
	if xgcerp_number in convert:
		return convert[xgcerp_number]
...
def get_asset_naming_series():
	from xgcerp.assets.doctype.asset.asset import get_asset_naming_series
