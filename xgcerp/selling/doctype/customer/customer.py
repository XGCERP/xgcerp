...
from xgcerp.accounts.party import get_dashboard_info, validate_party_accounts
from xgcerp.controllers.website_list_for_contact import add_role_for_portal_user
...
class Customer(TransactionBase):
	...
	if TYPE_CHECKING:
		...
		from xgcerp.accounts.doctype.allowed_to_transact_with.allowed_to_transact_with import (
			AllowedToTransactWith,
		)
		from xgcerp.accounts.doctype.party_account.party_account import PartyAccount
		from xgcerp.selling.doctype.customer_credit_limit.customer_credit_limit import (
			CustomerCreditLimit,
		)
		from xgcerp.selling.doctype.sales_team.sales_team import SalesTeam
		from xgcerp.utilities.doctype.portal_user.portal_user import PortalUser
	...
	def copy_communication(self):
		...
		from xgcerp.crm.utils import copy_comments, link_communications
	...
	if credit_limit > 0 and flt(customer_outstanding) > credit_limit:
		...
		if not credit_controller_role or credit_controller_role not in frappe.get_roles():
			...
			frappe.msgprint(
				...
				primary_action={
					"label": "Send Email",
					"server_action": "xgcerp.selling.doctype.customer.customer.send_emails",
