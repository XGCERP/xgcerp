...
frappe.provide("xgcerp.selling");

xgcerp.sales_common = {
	setup_selling_controller: function () {
		xgcerp.selling.SellingController = class SellingController extends xgcerp.TransactionController {
			setup() {
				super.setup();
			...
			setup_queries() {
				...
				$.each(
					...
					function (i, opts) {
						if (me.frm.fields_dict[opts[0]]) me.frm.set_query(opts[0], xgcerp.queries[opts[1]]);
					}
				);

				me.frm.set_query("contact_person", xgcerp.queries.contact_query);
				me.frm.set_query("company_contact_person", xgcerp.queries.company_contact_query);
				me.frm.set_query("customer_address", xgcerp.queries.address_query);
				me.frm.set_query("shipping_address_name", xgcerp.queries.address_query);
				me.frm.set_query("dispatch_address_name", xgcerp.queries.dispatch_address_query);
				me.frm.set_query("company_address", xgcerp.queries.company_address_query);
			...
			set_default_company_address() {
				...
				if (this.frm.doc.company) {
					frappe.call({
						method: "xgcerp.setup.doctype.company.company.get_default_company_address",
						args: {
			...
			customer_address() {
				xgcerp.utils.get_address_display(this.frm, "customer_address");
				xgcerp.utils.set_taxes_from_address(
					this.frm,
					"customer_address",
					"customer_address",
			...
			shipping_address_name() {
				xgcerp.utils.get_address_display(this.frm, "shipping_address_name", "shipping_address");
				xgcerp.utils.set_taxes_from_address(
					this.frm,
					"shipping_address_name",
					"customer_address",
			...
			dispatch_address_name() {
				xgcerp.utils.get_address_display(this.frm, "dispatch_address_name", "dispatch_address");
			}
			...
			pick_serial_and_batch(doc, cdt, cdn) {
				...
				frappe.db.get_value("Item", item.item_code, ["has_batch_no", "has_serial_no"]).then((r) => {
					if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
						...
						new xgcerp.SerialBatchPackageSelector(me.frm, item, (r) => {
							if (r) {
			...
			project() {
				...
				if (["Delivery Note", "Sales Invoice", "Sales Order"].includes(this.frm.doc.doctype)) {
					if (this.frm.doc.project) {
						frappe.call({
							method: "xgcerp.projects.doctype.project.project.get_cost_center_name",
