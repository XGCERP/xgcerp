...
frappe.setup.on("before_load", function () {
	if (
		...
		frappe.boot.setup_wizard_completed_apps.includes("xgcerp")
	) {
		return;
	}

	xgcerp.setup.slides_settings.map(frappe.setup.add_slide);
});

xgcerp.setup.slides_settings = [
	{
		// Organization
		name: "organization",
		...
		set_fy_dates: function (slide) {
			...
			if (country) {
				let fy = xgcerp.setup.fiscal_years[country];
		...
		load_chart_of_accounts: function (slide) {
			...
			if (country) {
				frappe.call({
					method: "xgcerp.accounts.doctype.account.chart_of_accounts.chart_of_accounts.get_charts_for_country",
		...
		charts_modal: function (slide, chart_template) {
			...
			let coa_tree = new frappe.ui.Tree({
				...
				label: parent,
				expandable: true,
				method: "xgcerp.accounts.utils.get_coa",
				args: {
