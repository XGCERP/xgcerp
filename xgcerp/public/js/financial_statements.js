frappe.provide("xgcerp.financial_statements");

xgcerp.financial_statements = {
	filters: get_filters(),
	baseData: null,
	formatter: function (value, row, column, data, default_formatter, filter) {
		...
		if (data && column.fieldname == this.name_field) {
			...
			if (data.account || data.accounts) {
				column.link_onclick =
					"xgcerp.financial_statements.open_general_ledger(" + JSON.stringify(data) + ")";
			}
			column.is_tree = true;
		}
	...
	onload: function (report) {
		...
		xgcerp.financial_statements.filters = get_filters();

		let fiscal_year = xgcerp.utils.get_fiscal_year(frappe.datetime.get_today());
...
function get_filters() {
	let filters = [
		...
		// accordingly.
		{
			...
			label: __("Currency"),
			fieldtype: "Select",
			options: xgcerp.get_presentation_currency_list(),
		},
		{
	...
	let fiscal_year = xgcerp.utils.get_fiscal_year(frappe.datetime.get_today(), false, true);
	if (fiscal_year) {
		let fy = xgcerp.utils.get_fiscal_year(frappe.datetime.get_today(), false, false);
