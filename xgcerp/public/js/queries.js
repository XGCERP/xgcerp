...
frappe.provide("xgcerp.queries");
$.extend(xgcerp.queries, {
	user: function () {
	...
	item: function (filters) {
		var args = { query: "xgcerp.controllers.queries.item_query" };
		if (filters) args["filters"] = filters;
		return args;
	},

	bom: function () {
		return { query: "xgcerp.controllers.queries.bom" };
	},

	task: function () {
		return { query: "xgcerp.projects.utils.query_task" };
	},
	...
	employee: function () {
		return { query: "xgcerp.controllers.queries.employee_query" };
	},
	...
	get_filtered_dimensions: function (doc, child_fields, dimension, company) {
		...
		return {
			query: "xgcerp.controllers.queries.get_filtered_dimensions",
			filters: {
				dimension: dimension,
				account: account,
				company: company,
...
xgcerp.queries.setup_queries = function (frm, options, query_fn) {
	var me = this;
...
*/
xgcerp.queries.setup_warehouse_query = function (frm) {
	frm.set_query("warehouse", "items", function (doc, cdt, cdn) {
		var row = locals[cdt][cdn];
		var filters = xgcerp.queries.warehouse(frm.doc);
		if (row.item_code) {
			$.extend(filters, { query: "xgcerp.controllers.queries.warehouse_query" });
