...
This design covers the systematic rebranding of the ERP application from the old identity (XGC Software Inc. / AXERP / xgccorp.com) to the new identity (Axina Group Inc. / AXERP / axinagroup.com). The scope is strictly limited to text replacements across 10 existing files plus the deletion of one stale logo file. No new modules, database schemas, APIs, or runtime logic are introduced.
...
4. **One Python class name** (`AXERPAddress` → `AXERPAddress`)
...
|-----------|-----------|---------|
| `AXERP` | `AXERP` | Product brand name |
...
| `AXERPAddress` | `AXERPAddress` | Python class name |
| `xgcerp-logo.svg` | `axinagroup-logo.svg` | Logo filename reference |
| `https://github.com/AXERP/xgcerp` | `https://github.com/AXERP/axerp` | Source link |
...
|-------------|-----|-----|
| `app_title` | `"AXERP"` | `"AXERP"` |
| `app_publisher` | `"XGC CORP."` | `"Axina Group Inc."` |
| `app_email` | `"db@xgccorp.com"` | `"db@axinagroup.com"` |
| `source_link` | `"https://github.com/AXERP/xgcerp"` | `"https://github.com/AXERP/axerp"` |
| `extend_doctype_class` | `AXERPAddress` | `AXERPAddress` |
| `default_mail_footer` | `AXERP` | `AXERP` |
| Global Search comment | `# AXERP doctypes` | `# AXERP doctypes` |
...
| Target | Old | New |
|--------|-----|-----|
| All brand name occurrences | `AXERP` | `AXERP` |
| Logo alt text | `AXERP Logo` | `AXERP Logo` |
...
| `source_link` in `update_hooks_metadata` | `"https://github.com/AXERP/xgcerp"` | `"https://github.com/AXERP/axerp"` |
...
| Target | Old | New |
|--------|-----|-----|
| Class name | `AXERPAddress` | `AXERPAddress` |
...
| Residual old brand strings remain after replacement | Post-change grep/search for `AXERP`, `xgccorp.com`, `XGC CORP.` across affected files catches any missed replacements. |
...
- [ ] Old brand string (`AXERP`) is absent
