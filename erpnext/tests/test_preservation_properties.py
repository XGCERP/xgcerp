...
class TestPreservationProperties(unittest.TestCase):
	...
	def test_09_smart_rename_preserves_file_structure(self):
		...
		# Check that erpnext_integrations folder exists (not renamed to xgcerp_integrations)
		self.assertTrue(
			self.integrations_folder.exists(),
			"erpnext_integrations folder should exist (not renamed)"
		)
		
		# Check that no xgcerp_integrations folder exists
		xgcerp_folder = self.repo_root / "erpnext" / "xgcerp_integrations"
		self.assertFalse(
			xgcerp_folder.exists(),
			"xgcerp_integrations folder should NOT exist (folders are not renamed)"
		)
